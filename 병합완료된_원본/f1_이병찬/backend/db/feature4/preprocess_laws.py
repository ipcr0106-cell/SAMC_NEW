...
def chunk_by_article(
    text: str, law_name: str, 고시번호: str, tier: int = TIER_고시
) -> list[dict]:
    """
    조문(제N조) 단위로 청킹.
    ...
    while i < len(parts):
        if _ARTICLE_RE.fullmatch(parts[i].strip()):
            ...
            if i < len(parts):
                body = parts[i].strip()
                sub_chunks = _split_if_long(
                    body, current_article, law_name, 고시번호, tier
                )
            ...
            if body:
                chunks.append(
                    _make_chunk(body, current_article, law_name, 고시번호, tier)
                )
        i += 1
...
def _split_if_long(
    body: str, article: str, law_name: str, 고시번호: str, tier: int = TIER_고시
) -> list[dict]:
    """조문 내용이 길면 항(①②③) 단위로 재분할."""
    ...
    for part in parts:
        if _PARA_RE.fullmatch(part):
            if current_para:
                ...
                sub_chunks.append(
                    _make_chunk(current_para.strip(), label, law_name, 고시번호, tier)
                )
            current_num = part
            current_para = part
        else:
    ...
    return (
        sub_chunks
        if sub_chunks
        else [_make_chunk(body, article, law_name, 고시번호, tier)]
    )


def _make_chunk(
    text: str, 조문번호: str, law_name: str, 고시번호: str, tier: int = TIER_고시
) -> dict:
    return {
...
def upsert_to_pinecone(
    ...
    for i in tqdm(
        range(0, len(records), UPSERT_BATCH), desc="  Pinecone 적재", leave=False
    ):
...
def save_law_document(supabase_client, law_info: dict, total_chunks: int) -> str:
    ...
    if existing.data:
        ...
        supabase_client.table("f4_law_documents").update(payload).eq(
            "id", doc_id
        ).execute()
        return doc_id
    else:
...
def main() -> None:
    ...
    for law_info in LAW_FILES:
        ...
        chunks = chunk_by_article(
            text, law_info["law_name"], law_info["고시번호"], tier
        )
        article_cnt = len(chunks)
        ...
        for t in table_texts:
            chunks.append(
                _make_chunk(
                    t, "별표/표", law_info["law_name"], law_info["고시번호"], tier
                )
            )
        ...
        for t in image_texts:
            chunks.append(
                _make_chunk(
                    t, "별표/도안", law_info["law_name"], law_info["고시번호"], tier
                )
            )

        print(
            f"     → 조문 {article_cnt}개 + 표 {len(table_texts)}개 + 이미지 {len(image_texts)}개 = 총 {len(chunks)}개 청크"
        )
