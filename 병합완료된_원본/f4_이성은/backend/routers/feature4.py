...
# =============================================================
# OCR 연동 테이블/컬럼 설정  (OCR 팀 스키마 확정 시 여기만 수정)
# =============================================================

# 회사 제출 서류 OCR 결과 테이블 — 제품명·원재료·내용량·원산지·제조사
_DOC_OCR_TABLE = "document_ocr_results"
_DOC_OCR_COLUMNS = {
    "case_id":        "case_id",
...
# 라벨 이미지 OCR 결과 테이블 — 라벨에서 추출한 텍스트·제품명 등
_LABEL_OCR_TABLE = "label_ocr_results"
_LABEL_OCR_COLUMNS = {
    "case_id":        "case_id",
...
def _get_clients() -> dict:
    ...
    required = ["PINECONE_API_KEY", "PINECONE_HOST", "SUPABASE_URL", "SUPABASE_SERVICE_KEY", "OPENAI_API_KEY"]
    missing = [k for k in required if not os.getenv(k)]
    if missing:
        raise RuntimeError(f".env 누락: {missing}")

    pc = Pinecone(api_key=os.getenv("PINECONE_API_KEY"))
    _clients["index"]    = pc.Index(host=os.getenv("PINECONE_HOST"))
    ...
    _clients["claude"]   = OpenAI(api_key=os.getenv("OPENAI_API_KEY"))
    return _clients
...
def _fetch_doc_ocr(supabase, case_id: str) -> dict:
    """회사 제출 서류 OCR 결과를 DB에서 조회. 없으면 빈 dict 반환."""
    try:
        cols = _DOC_OCR_COLUMNS
        res = (
            supabase.table(_DOC_OCR_TABLE)
            .select(f"{cols['product_name']},{cols['ingredients']},{cols['content_volume']},{cols['origin']},{cols['manufacturer']}")
        ...
        if res.data:
            row = res.data[0]
            return {
                "product_name":   row.get(cols["product_name"], ""),
                "ingredients":    row.get(cols["ingredients"], ""),
                "content_volume": row.get(cols["content_volume"], ""),
                "origin":         row.get(cols["origin"], ""),
                "manufacturer":   row.get(cols["manufacturer"], ""),
            }
    except Exception:
        pass
    return {}
...
def _fetch_label_ocr(supabase, case_id: str) -> dict:
    """라벨 이미지 OCR 결과(텍스트·제품명 등)를 DB에서 조회. 없으면 빈 dict 반환."""
    try:
        cols = _LABEL_OCR_COLUMNS
        res = (
            supabase.table(_LABEL_OCR_TABLE)
            .select(f"{cols['label_text']},{cols['product_name']},{cols['ingredients']},{cols['content_volume']},{cols['origin']},{cols['manufacturer']}")
            .eq(cols["case_id"], case_id)
            .limit(1)
            .execute()
        )
        if res.data:
            row = res.data[0]
            return {
                "label_text":     row.get(cols["label_text"], ""),
                "product_name":   row.get(cols["product_name"], ""),
                "ingredients":    row.get(cols["ingredients"], ""),
                "content_volume": row.get(cols["content_volume"], ""),
                "origin":         row.get(cols["origin"], ""),
