...
def _get_clients() -> dict:
    ...
    _clients["pinecone"] = Pinecone(api_key=os.getenv("PINECONE_API_KEY"))
...
def _search_pinecone(query_vec: list[float], clients: dict, top_k: int = 8) -> list[dict]:
    ...
    index_name = os.getenv("PINECONE_INDEX", "samc-a")
...
async def run_feature2(case_id: str):
    """
    기능2 실행: 파싱된 원재료 텍스트 → 식품유형 AI 분류 → pipeline_steps 저장
    """
    ...
    # 2. 파싱된 원재료 문서 조회 (ingredients 우선, 없으면 전체)
    docs_res = (
        sb.table("documents")
    ...
    if not selected_doc or not selected_doc.get("parsed_md"):
        raise HTTPException(status_code=400, detail="파싱된 문서가 없습니다. 먼저 서류를 파싱하세요.")

    parsed_text = selected_doc["parsed_md"]
